...
const imageUrl = ref('');
const formKeys = computed(() => Object.keys(formAnamnesys.value) as (keyof AnamnesysForm)[]);
const file_path = ref('');
...
const createSummary = (form: typeof formAnamnesys.value): string => {
  return `Pasien dengan jenis kelamin: ${form.jenis_kelamin}, ` +
...
const getInputType = (key: keyof AnamnesysForm): string => {
  if (key.includes('riwayat') || key.includes('keluhan') || key.includes('kriteria')) return 'textarea';
...
const handleSubmit = async (): Promise<void> => {
  if (true) { // validateForm()
    try {
      const externalUrl = `${runtimeConfig.public.backendUrl}/predict`;
      const res = await $fetch(externalUrl, {
        method: 'POST',
        body: {
...
<template>
  <div class="mx-auto flex flex-col items-center justify-evenly">
    <div class="container flex flex-col items-center justify-evenly p-4">
      ...
      <Icon v-if="isLoading" name="eos-icons:three-dots-loading" size="100" style="color: black" />
      <div class="flex flex-col items-center w-full justify-center">

        <label class="block text-xl font-semibold text-gray-800 mb-4 cursor-pointer
        ...
        <input
            ...
            accept="image/*"
            @change="onFileChange"
            class="w-full max-w-sm cursor-pointer rounded-lg border border-gray-300
         p-3 text-gray-700 shadow-sm
        ...
        <div v-if="imageUrl" v-for="key in formKeys" class="flex items-center justify-between w-full py-2 px-8">
          <label class="w-[20%] font-bold" :for="key">{{ formatLabel(key) }}</label>
        ...
        <button @click="handleSubmit" v-if="imageUrl" :disabled="isLoading" :class="isLoading?'bg-gray-400':'bg-blue-400'" class="py-2 px-8 h-full rounded-tr-lg rounded-lg my-8">Send</button>
      </div>
    </div>
  </div>
    ...
    >
      <div class="bg-white rounded-lg shadow-lg p-6 max-w-xl w-full relative">
        ...
        <p class="mb-2 text-xl">
          Klasifikasi penyakit termasuk <strong>{{ result?.classname }} ({{result?.classname=="DA"?"Dermatitis Atopik":"Non Dermatitis Atopik"}})</strong>
        </p>
        <p class="text-xl">
          Model AI menghasilkan jawaban tersebut dengan nilai prosentase <strong>{{ (result?.confidence).toFixed(2) }} %</strong>
        </p>
      </div>
    </div>
  </transition>
